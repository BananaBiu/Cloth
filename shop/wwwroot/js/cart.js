$(document).ready(function () {
    ...
    function getSubTotal(row) {
        var price = parseFloat($(row).find(".realprice").text());
        ...
        $(row).find(".pricetotal").text(result).data("bind", result);
    };
    ...
    function getTotal() {
        ...
        $(cartTable).find("tr:gt(0)").each(function () {
            ...
            if ($(this).find(":checkbox").prop("checked") == true) {
                ...
                priceTotal += parseFloat($(this).find(".pricetotal").text());
            }
        });
        $("#itemCount").text(itemCount).data("bind", itemCount);
        $("#qtyCount").text(qtyTotal).data("bind", qtyTotal);
        $("#priceTotal").text(priceTotal).data("bind", priceTotal);
    };
    ...
    $(cartTable).find("tr:gt(0)").each(function () {
        ...
        $(this).click(function () {
            ...
            else if ($(window.event.srcElement).hasClass("collect")) {
                if (confirm("确定要收藏此产品？")) {
                    $(this).remove();
                }
            }
